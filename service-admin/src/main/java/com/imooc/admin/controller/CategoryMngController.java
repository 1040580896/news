...
public class CategoryMngController extends BaseController implements CategoryMngControllerApi {
    ...
    public GraceJSONResult getCats() {
        ...
        if (StringUtils.isBlank(allCatJson)) {
            ...
            redis.set(REDIS_ALL_CATEGORY, JsonUtils.objectToJson(categoryList), 600);
        } else {
     ...
     */
    @Override
    public GraceJSONResult deleteCats(Integer cid) {
        if (cid == null) return GraceJSONResult.errorMsg("文章ID不能为空");
        Integer res = categoryService.deleteCats(cid);
        if(res==1){

            return GraceJSONResult.ok("删除成功");
        }
        return GraceJSONResult.errorMsg("删除失败");
